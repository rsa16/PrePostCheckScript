...
def sv_upgrade_preChecks():
    ...
    commandOutputs.update({result.command: result.stdout})
    result = subprocess.run(['curl', '-I', '-k', '-L', '-x', 'injr-cbc1:81', 'http://apple.com'], capture_output=True, text=True)
    commandOutputs.update({' '.join(result.args): result.stdout})
    result = subprocess.run(['curl', '-k', '-x', 'injr-cbc1:81', 'http://www.eicar.org/download/eicar.com.txt'], capture_output=True, text=True)
    commandOutputs.update({' '.join(result.args): result.stdout})
    ...
    cmd7 = [commandOutputs['$curl -I -k -L -x injr-cbc1:81 http://apple.com'].split('\n')[14]] + [commandOutputs['$curl -I -k -L -x injr-cbc1:81 http://apple.com'].split('\n')[16]]
    cmd8 = commandOutputs['$curl -k -x injr-cbc1:81 http://www.eicar.org/download/eicar.com.txt'].split('\n')[2].strip()
    ...
    print('RUNNING COMMAND - curl -I -k -L -x injr-cbc1:81 http://apple.com')
    print('COMMAND OUTPUT: ', end='')
    error = None
    for i in cmd7:
    ...
    if error != None:
        print('\n')
        print(f'ERROR: {error}')
        failed.append('curl -I -k -L -x injr-cbc1:81 http://apple.com')
        print('-------------------------- TEST FAILED -------------------------------\n\n')
    else:
        passed.append('curl -I -k -L -x injr-cbc1:81 http://apple.com')
        print('\n', end='')
        print('-------------------------- TEST PASSED -------------------------------\n\n')

    print('RUNNING COMMAND - curl -k -x injr-cbc1:81 http://www.eicar.org/download/eicar.com.txt')
    print(f'COMMAND OUTPUT: {cmd8}')

    if not cmd8 == '<title>Bloomberg Proxy Exception Page - Access Denied </title>':
        print(f'ERROR: EIGTH COMMAND DOES NOT EQUAL CORRECT VALUE. VALUE IS {cmd8}')
        failed.append('curl -k -x injr-cbc1:81 http://www.eicar.org/download/eicar.com.txt')
        print('-------------------------- TEST FAILED -------------------------------\n\n')
    else:
        passed.append('curl -k -x injr-cbc1:81 http://www.eicar.org/download/eicar.com.txt')
        print('-------------------------- TEST PASSED -------------------------------\n\n')
